...
1. Applying `ResourceQuota` and `LimitRange` to each namespace
1. Applying `NetworkPolicy` resources for intra-tenant and external-egress rules
1. Creating `RoleBinding` resources for declared users and groups
1. Applying all referenced namespace templates (deploying standard tooling)
